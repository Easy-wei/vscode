...
def compass (a,b):
    c= []
    for i in range(0,len(b['query_result']['data']['rows'])):
        sub = b['query_result']['data']['rows'][i]['codenum']- a['query_result']['data']['rows'][i]['codenum']
        if sub >=10:
            # 把【device_id， 筐数，1min报错数量】以list形式写入到c，然后return
            c.append([b['query_result']['data']['rows'][i]['device_id'],b['query_result']['data']['rows'][i]['basket_count'],sub])
    a = b # 把result1更新了
    return c 

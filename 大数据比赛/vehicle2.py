...
def to_list(nrows):
    data = xlrd.open_workbook(r'E:\code\vscode\大数据比赛重做版\vehicle2.xlsx')
...
def to_str(x):
    ...
    for i in x:
        i = round(i)
        list_i.append(
            int(time.mktime(time.strptime(str(i), '%Y%m%d%H%M%S'))))
    ...
    try:
        ...
        if response.json()['status'] != 0:         
            ...
            if response.json()['status'] == 302:# 等于这个，则意味着今天测数数量用完了
                with open('vechicle2_post_row_num.text', 'a+') as f1:
                    f1.write(str(i)+'\n'+response.text+'\n')
                break
